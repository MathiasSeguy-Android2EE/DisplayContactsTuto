 ...
 *        </ul>
 */
public enum ContactProviderLegacy implements ContentProviderIntf {
	...
	public List<SimpleContact> getAllContact() {
		...
		while (contactCursor.moveToNext()) {
			...
			current = new SimpleContact(id, "type", name, hasPhoneNum, photoId, null);
	...
	public List<SimpleContact> getPhoneContactFaster(boolean phoneContactOnly) {
		...
		int idColIndex = contactCursor.getColumnIndexOrThrow(ContactsContract.Contacts._ID);
		// the data to retrieve
		String id, photoId, name, hasPhoneNum;
		// create the return list
		List<SimpleContact> result = new ArrayList<SimpleContact>(contactCursor.getCount());
		// the current contact
		SimpleContact current;
	...
	public void browseCalendar(Context ctx) {
		// This method can only be done with IceCreamSandwich or higher
		if (!ctx.getResources().getBoolean(R.bool.postIceCreamSandwich)) {
			Toast.makeText(ctx, "This method (browseCalendar) can only be execute with IceCreamSandwich or higher",
